...
std::string DEFAULT_TEXT_EDITOR = "nano";
...
void showVersion() {
    std::cout << "Current Taskpp version: " << VERSION << " for Linux Systems\n\n";
}


void loadData() {
    char buffer[4096];
    ssize_t len = readlink("/proc/self/exe", buffer, 4096 - 1);
    ...
    if (len != -1) {
        ...
        if (pos != std::string::npos) {
            ...
            // Handle error: unable to extract directory from executable path
        }
    } else {
        // Handle error: unable to read symlink for executable path
    }
}


void addTask(const std::string& name, char* argv[]) {
    std::string filename = DEFAULT_DIRECTORY + name + ".sh";
    ...
    result = std::system(chmodCommand.c_str());
    if (result != 0) {
        std::cerr << "Error setting execute permission for the script.\n\n";
        file.close();
        return;
    }
...
void runTask(const std::string& name) {
    std::string filePath = DEFAULT_DIRECTORY + name + ".sh";
