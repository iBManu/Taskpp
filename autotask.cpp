...
const std::string DEFAULT_DIRECTORY = "D:/Autotask/";
const std::string DATAFILE_NAME = DEFAULT_DIRECTORY + "dataFile.txt";
const std::string VERSION = "1.0.0";
std::string DEFAULT_TEXT_EDITOR = "notepad.exe";

void showHelp() {
    std::cout << "Available commands and usage:\n\n"
              << "- autotask add \"name\" [\"description\"]: Creates a new task.\n"
              << "- autotask run \"name\": Executes the task with the given name.\n"
              << "- autotask remove \"name\": Removes the task with the given name.\n"
              << "- autotask edit \"name\": Opens the given task file for editing.\n"
              << "- autotask list: Lists all available tasks.\n"
              << "- autotask help: Shows available commands and usage.\n"
              << "- autotask version: Shows the version of Autotask.\n"
              << "- autotask info: Shows information about Autotask.\n\n";
}

void showInfo() {
    std::cout << "Autotask is simple and open source program for automatizing multiple cli tasks\n"
              << "It is coded in C++\n"
              << "Created by Manuel Camacho Campos (github.com/ibmanu)\n\n";
}

void showVersion() {
    std::cout << "Current Autotask version: " << VERSION << " for Windows Systems\n\n";
}
...
void addTask(const std::string& name, char* argv[]) {
    ...
    if(file.good()) {
        std::cout << "A task with the same name already exists, if you want to edit it type autotask edit " + name << "\n\n";
        file.close();
        return;
    }
...
void editTask(const std::string& name) {
    ...
    if(!file.good()) {
        std::cout << "A task with that name doesn't exists, if you want to add a task type autotask add " + name << "\n\n";
        file.close();
        return;
    }
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cerr << "Usage: autotask <command> [arguments]" << std::endl;
        std::cout << "If you need help, type \"autotask help\"\n\n";
        return 1;
    }
